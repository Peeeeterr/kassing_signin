# 学搭子自动化打卡助手 (kassing-signin) v1.2.2

> 作者：@护盾电池  
> 项目仓库：https://github.com/Peeeeterr/kassing_signin

基于逆向分析学搭子平台前端渲染与网络通信协议构建的自动化签到工具。实现时段自动检测、定位离散采样、自适应水印合成、滑动窗口照片冷却与打卡闭环。

---

## 核心机制

- 动态照片冷却：单参数控制冷却期（最低 9 次，覆盖 3 天轮转），强制图库图片数量达到「冷却池 + 8 张」，规避图像重复审查。
- 智能单次/计数跳过打卡：支持快捷跳过单次打卡或自定义指定次数打卡（默认 1 次），到点自动安全跳过并递减计数，配额归零后自愈恢复正常打卡，避免临时请假或手动打卡后的重复提交。
- 定时器全链路体检诊断：内置 6 大项定时器自动化体检，覆盖守护进程状态、时区时钟偏移、定时规则解析、脚本权限与环境依赖，并支持全链路仿真触发执行。
- 极坐标离散定位：在允许半径内均匀生成经纬度并附带随机精度 Jitter，模拟真实移动端定位。
- 水印与 EXIF 自适应：对齐官方 Canvas 阵列水印排版，自动纠正手机横竖屏朝向，并动态控制图片体积在 1MB 以内。
- 跨平台防风控调度：内置 Linux (crontab) 与 Windows (Task Scheduler) 启动脚本，加入随机延时（Jitter）杜绝固定秒级特征。
- 东八区强制锁定：内部时间解析与接口格式化强制锁定 UTC+8（中国标准时间），消除服务器时区配置差异。

---

## 项目结构

```text
kassing-signin/
├── run.sh                  Linux/macOS 统一控制台管理与启动入口
├── run.bat                 Windows 统一控制台管理批处理（支持双击即用）
├── windows.bat             Windows 双击运行兜底保护脚本（异常自动暂停防闪退）
├── main.py                 主运行程序（支持两阶段前置检测、倒计时与参数控制）
├── kassing_signin/         核心功能模块包
│   ├── __init__.py
│   ├── config.py           配置解析、极坐标离散算法与冷却池调度
│   ├── kassing_api.py      官方接口通信（登录鉴权、时段查询、对象存储上传、提交打卡）
│   ├── watermark.py        EXIF 朝向校正与 Canvas 阵列水印合成引擎
│   └── scheduler.py        跨平台系统定时任务自动化读写管理引擎 (Linux / Windows)
├── scripts/                测试诊断与演练工具集
│   ├── check_status.py     查看今日打卡状态与图库健康度
│   ├── test_watermark.py   测试本地图片防伪水印渲染
│   ├── dry_run_signin.py   演练打卡全流程（Dry-run 保护模式）
│   ├── test_timer.py       系统定时器健康诊断与全链路测试
│   └── menu_console.py     测试工具综合交互控制台
├── PhotoStorage/           待打卡图片仓库（需存放 冷却池 + 8 张日常照片）
├── Archives/               打卡生成水印照片归档目录
├── logs/                   自动打卡执行日志目录 (cron.log)
├── run_cron.sh             Linux 后台定时执行脚本（供 crontab 调度）
├── run_windows.bat         Windows 后台定时执行批处理
├── run_silent.vbs          Windows 后台静默无弹窗运行脚本
├── requirements.txt        Python 第三方依赖清单
├── .env.example            环境配置文件模版
├── CHANGELOG.md            项目版本迭代日志
└── README.md               项目说明文档
```

---

## 配置与使用指引

### 天才第零步：点个Star
项目仓库：https://github.com/Peeeeterr/kassing_signin

如果你遇到了配置文件错误的问题，请到该仓库下载最新版。如果问题没有解决，请到该仓库提交Issues。

如果有帮你节省时间，希望你能来点一个Star。

### 步骤一：一键运行与初始化向导
程序要求 Python 3.9+ 环境，首次启动时会自动检测并安装所需依赖。直接启动统一入口脚本即可拉起向导：
- **Linux 用户**：
  ```bash
  ./run.sh
  ```
- **Windows 用户**：直接双击 `run.bat`（或在终端运行 `run.bat`）

向导将依次引导完成以下配置：
1. 账号与密码：输入学搭子登录用户名及密码；
2. 冷却池配置：设定照片冷却期次数（系统最低要求 9 次，直接回车默认 9）；
3. 图片准备与校验：将日常照片拷贝至 `PhotoStorage/` 目录（数量需达到「冷却池 + 8 张」，默认至少 17 张；注意尽量避免包含窗外日光，以防夜间打卡违和），按回车自动校验照片数量；
4. 运行参数确认：确认定位半径（建议 10 ~ 150 米，直接回车默认 50 米）；
5. 自动生成 `.env`：保存并完成本地凭据初始化；
6. **一键写入系统定时任务（可选）**：向导会自动联网拉取实际打卡时段并推荐打卡时间，回车即可一键写入操作系统定时器（Linux 写入 crontab，Windows 注册任务计划），彻底免去手动配置。

### 步骤二：日常运行与控制台管理
日常管理推荐直接运行统一入口脚本，程序会自动弹出**全功能交互式控制台菜单**：
- **Linux 用户**：终端执行 `./run.sh`
- **Windows 用户**：直接双击 `run.bat`

在控制台中，只需输入数字编号即可直观执行各项功能：
```text
  【打卡服务】
    [1] 常规签到
    [2] 查看今日签到记录与状态

  【自动打卡与假期管理】
    [3] 暂停自动打卡
    [4] 恢复自动打卡
    [5] 开启 / 修改自动打卡时间
    [6] 关闭 / 卸载自动打卡任务
    [7] 查看自动打卡状态与运行日志
    [8] 跳过下次打卡

  【测试与演练工具】
    [9] 演练打卡全流程
   [10] 测试本地水印合成
   [11] 诊断账号状态与图库健康度
   [12] 测试系统定时器与调度健康度

  【设置与维护】
   [13] 重新运行配置向导
   [14] 检查并修复运行环境

    [0] 退出控制台
```

### 进阶：命令行参数
支持向入口脚本（Linux: `./run.sh <参数>`，Windows: `run.bat <参数>`）直接传递以下参数：
- `-y`：立即打卡
- `-records`：查询今日记录
- `-status`：查看定时状态与下次预计触发时间
- `-test-cron`：一键全面诊断定时器健康度并进行仿真触发测试
- `-skip`：快捷跳过单次打卡（下次触发时自动跳过 1 次并自动恢复）
- `--skip 3`：跳过后续指定次数打卡（如 3 次，递减归零后恢复）
- `-cancel-skip`：取消已设定的跳过打卡任务
- `-pause`：暂停自动打卡
- `-resume`：恢复自动打卡（同时清除暂停与跳过状态）
- `-setup-cron`：修改定时时间
- `-remove-cron`：卸载定时任务
- `--dry-run`：演练打卡全流程
- `--force`：强制打卡
- `--slot 晚自习`：指定打卡时段

---

## 运行日志查看
所有通过定时脚本执行的打卡记录、定位坐标、随机延时及服务器响应均自动记录于 `logs/cron.log`。
查看最新执行日志：
- Linux 终端：`tail -n 50 logs/cron.log`
- Windows：使用文本编辑器打开 `logs\cron.log`
